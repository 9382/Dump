...
_getUntilEnding = lambda character,opener : regex.compile(f"(.*?)(\\{opener}[^{character}]+\\{character}.*?\2?[^\\\\]?|[^\\\\])(\\{character}[\s\S]*)")
...
def _compareReady(reg1,reg1Checks,reg2,reg2Checks,printResult=True):
    # print(reg1,reg2)
    r1 = regex.compile(reg1)
    r2 = regex.compile(reg2)
    if not reg1Checks or not reg2Checks:
...
def compareSet(regset,printResult=True):
    ...
    for i in range(len(regset)):
        print(i,"out of",len(regset))
        choice = regset[i]
        compiled[choice] = _getPotentials(choice)
    print("Comparing")
    for i in range(len(regset)):
        print(i,"out of",len(regset))
        for ii in range(len(regset)):
            if ii > i:
                inferior = _compareReady(regset[i],compiled[regset[i]],regset[ii],compiled[regset[ii]],printResult)
                if inferior == 1:
